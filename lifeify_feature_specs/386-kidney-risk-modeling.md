# Feature 386: Kidney Risk Modeling
...
| Feature Key | `kidney_risk_modeling` |

## Purpose

Kidney Risk Modeling helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Kidney Risk Modeling` safely:
...
Show a calm empty state explaining that no `Kidney Risk Modeling` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
