# Feature 402: RAG / Medical Knowledge Retrieval
...
| Feature Number | 402 |
| Version | V3 |
| Category | Knowledge Retrieval & Embeddings |
| Feature Key | `rag_medical_knowledge_retrieval` |

## Purpose

RAG / Medical Knowledge Retrieval helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `RAG / Medical Knowledge Retrieval` safely:
...
Additional fields and entities should be scoped to the category `Knowledge Retrieval & Embeddings` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `RAG / Medical Knowledge Retrieval` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
