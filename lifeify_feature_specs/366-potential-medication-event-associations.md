# Feature 366: Potential Medication-Event Associations
...
| Feature Key | `potential_medication_event_associations` |

## Purpose

Potential Medication-Event Associations helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Potential Medication-Event Associations` safely:
...
Show a calm empty state explaining that no `Potential Medication-Event Associations` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
