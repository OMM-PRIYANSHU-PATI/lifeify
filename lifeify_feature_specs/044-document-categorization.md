# Feature 044: Document Categorization
...
| Feature Key | `document_categorization` |

## Purpose

Document Categorization helps organize medical documents and convert user-confirmed extracted information into structured records.
...
The feature requires the minimum structured data needed to represent `Document Categorization` safely:
...
Show a calm empty state explaining that no `Document Categorization` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
