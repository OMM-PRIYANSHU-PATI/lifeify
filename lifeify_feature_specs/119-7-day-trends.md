...
| Feature Key | `7_day_trends` |

## Purpose

7-Day Trends captures longitudinal health information so the user can build a useful timeline without depending on a single external platform.
...
The feature requires the minimum structured data needed to represent `7-Day Trends` safely:
...
Show a calm empty state explaining that no `7-Day Trends` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
