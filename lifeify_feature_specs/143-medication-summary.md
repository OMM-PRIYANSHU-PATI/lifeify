# Feature 143: Medication Summary
...
| Feature Key | `medication_summary` |

## Purpose

Medication Summary supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Medication Summary` safely:
...
Show a calm empty state explaining that no `Medication Summary` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
