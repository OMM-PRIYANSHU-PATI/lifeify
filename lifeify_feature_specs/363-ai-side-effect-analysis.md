# Feature 363: AI Side-Effect Analysis
...
| Feature Key | `ai_side_effect_analysis` |

## Purpose

AI Side-Effect Analysis helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `AI Side-Effect Analysis` safely:
...
Show a calm empty state explaining that no `AI Side-Effect Analysis` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
