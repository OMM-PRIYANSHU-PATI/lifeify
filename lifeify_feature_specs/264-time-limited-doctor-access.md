# Feature 264: Time-Limited Doctor Access
...
| Feature Key | `time_limited_doctor_access` |

## Purpose

Time-Limited Doctor Access lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Time-Limited Doctor Access` safely:
...
Show a calm empty state explaining that no `Time-Limited Doctor Access` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
