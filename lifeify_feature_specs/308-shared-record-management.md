# Feature 308: Shared Record Management
...
| Feature Key | `shared_record_management` |

## Purpose

Shared Record Management lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Shared Record Management` safely:
...
Show a calm empty state explaining that no `Shared Record Management` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
