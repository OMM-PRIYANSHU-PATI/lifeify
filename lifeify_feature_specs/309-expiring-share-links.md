# Feature 309: Expiring Share Links
...
| Feature Key | `expiring_share_links` |

## Purpose

Expiring Share Links lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Expiring Share Links` safely:
...
Show a calm empty state explaining that no `Expiring Share Links` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
