# Feature 390: Longitudinal Patient Summary
...
| Feature Key | `longitudinal_patient_summary` |

## Purpose

Longitudinal Patient Summary helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Longitudinal Patient Summary` safely:
...
Show a calm empty state explaining that no `Longitudinal Patient Summary` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
