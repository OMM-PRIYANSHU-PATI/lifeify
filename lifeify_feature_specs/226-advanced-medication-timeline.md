# Feature 226: Advanced Medication Timeline
...
| Feature Key | `advanced_medication_timeline` |

## Purpose

Advanced Medication Timeline supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Advanced Medication Timeline` safely:
...
Show a calm empty state explaining that no `Advanced Medication Timeline` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
