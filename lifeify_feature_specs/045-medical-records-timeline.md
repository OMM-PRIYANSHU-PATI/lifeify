# Feature 045: Medical Records Timeline
...
| Feature Key | `medical_records_timeline` |

## Purpose

Medical Records Timeline helps organize medical documents and convert user-confirmed extracted information into structured records.
...
The feature requires the minimum structured data needed to represent `Medical Records Timeline` safely:
...
Additional fields and entities should be scoped to the category `Medical Records` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Medical Records Timeline` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
