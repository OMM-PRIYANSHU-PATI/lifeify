# Feature 332: Contextual Symptom Questions
...
| Feature Number | 332 |
| Version | V3 |
| Category | AI Symptom Intelligence |
| Feature Key | `contextual_symptom_questions` |

## Purpose

Contextual Symptom Questions helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Contextual Symptom Questions` safely:
...
Additional fields and entities should be scoped to the category `AI Symptom Intelligence` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Contextual Symptom Questions` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
