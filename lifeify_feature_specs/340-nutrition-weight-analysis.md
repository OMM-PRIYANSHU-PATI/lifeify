# Feature 340: Nutrition/Weight Analysis
...
| Feature Key | `nutrition_weight_analysis` |

## Purpose

Nutrition/Weight Analysis helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Nutrition/Weight Analysis` safely:
...
Show a calm empty state explaining that no `Nutrition/Weight Analysis` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
