# Feature 358: Personalized Sleep Recommendations
...
| Version | V3 |
| Category | AI Sleep |
| Feature Key | `personalized_sleep_recommendations` |

## Purpose

Personalized Sleep Recommendations helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Personalized Sleep Recommendations` safely:
...
Additional fields and entities should be scoped to the category `AI Sleep` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Personalized Sleep Recommendations` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
