# Feature 370: Medication-Allergy Intelligence
...
| Feature Key | `medication_allergy_intelligence` |

## Purpose

Medication-Allergy Intelligence helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Medication-Allergy Intelligence` safely:
...
Show a calm empty state explaining that no `Medication-Allergy Intelligence` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
