# Feature 409: AI Feedback System
...
| Feature Key | `ai_feedback_system` |

## Purpose

AI Feedback System helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `AI Feedback System` safely:
...
Show a calm empty state explaining that no `AI Feedback System` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
