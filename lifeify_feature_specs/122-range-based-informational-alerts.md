# Feature 122: Range-Based Informational Alerts
...
| Feature Key | `range_based_informational_alerts` |

## Purpose

Range-Based Informational Alerts contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Range-Based Informational Alerts` safely:
...
Additional fields and entities should be scoped to the category `Chronic Health` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Range-Based Informational Alerts` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
