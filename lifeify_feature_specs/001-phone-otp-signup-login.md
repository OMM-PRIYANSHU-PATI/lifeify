# Feature 001: Phone OTP Signup/Login
...
| Feature Key | `phone_otp_signup_login` |

## Purpose

Phone OTP Signup/Login contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Phone OTP Signup/Login` safely:
...
Additional fields and entities should be scoped to the category `Account & Onboarding` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Phone OTP Signup/Login` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
