# Feature 345: AI Daily Health Briefing
...
| Feature Key | `ai_daily_health_briefing` |

## Purpose

AI Daily Health Briefing helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `AI Daily Health Briefing` safely:
...
Show a calm empty state explaining that no `AI Daily Health Briefing` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
