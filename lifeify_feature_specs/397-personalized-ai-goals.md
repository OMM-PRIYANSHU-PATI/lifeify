# Feature 397: Personalized AI Goals
...
| Feature Key | `personalized_ai_goals` |

## Purpose

Personalized AI Goals helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Personalized AI Goals` safely:
...
Show a calm empty state explaining that no `Personalized AI Goals` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
