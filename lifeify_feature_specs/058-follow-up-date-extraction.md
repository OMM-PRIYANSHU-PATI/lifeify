# Feature 058: Follow-up Date Extraction
...
| Feature Key | `follow_up_date_extraction` |

## Purpose

Follow-up Date Extraction helps organize medical documents and convert user-confirmed extracted information into structured records.
...
The feature requires the minimum structured data needed to represent `Follow-up Date Extraction` safely:
...
Show a calm empty state explaining that no `Follow-up Date Extraction` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
