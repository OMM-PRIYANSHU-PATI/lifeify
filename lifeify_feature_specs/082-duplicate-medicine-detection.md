# Feature 082: Duplicate Medicine Detection
...
| Feature Key | `duplicate_medicine_detection` |

## Purpose

Duplicate Medicine Detection supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Duplicate Medicine Detection` safely:
...
Additional fields and entities should be scoped to the category `Medication Safety` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Duplicate Medicine Detection` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
