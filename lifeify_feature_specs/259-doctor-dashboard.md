...
| Feature Key | `doctor_dashboard` |

## Purpose

Doctor Dashboard lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Doctor Dashboard` safely:
...
Show a calm empty state explaining that no `Doctor Dashboard` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
