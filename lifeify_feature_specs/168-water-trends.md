...
| Feature Key | `water_trends` |

## Purpose

Water Trends captures longitudinal health information so the user can build a useful timeline without depending on a single external platform.
...
The feature requires the minimum structured data needed to represent `Water Trends` safely:
...
- Quiz
- Voice
- Caregiver where permission exists

## Primary Data Source

Manual entry or gamified quiz.

## Fallback Data Source

Voice-to-structured draft with user confirmation.
...
A short quiz can collect the value, context, timing, severity, and optional notes.
...
Additional fields and entities should be scoped to the category `Basic Analytics` and linked by stable IDs with source metadata.
...
Health logs, normalized metrics, data provenance, sufficient history, and privacy settings.
...
Show a calm empty state explaining that no `Water Trends` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
