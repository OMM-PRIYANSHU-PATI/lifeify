# Feature 216: Indian Food Recommendations
...
| Feature Key | `indian_food_recommendations` |

## Purpose

Indian Food Recommendations contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Indian Food Recommendations` safely:
...
Show a calm empty state explaining that no `Indian Food Recommendations` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
