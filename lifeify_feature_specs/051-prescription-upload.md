# Feature 051: Prescription Upload
...
| Feature Key | `prescription_upload` |

## Purpose

Prescription Upload helps organize medical documents and convert user-confirmed extracted information into structured records.
...
The feature requires the minimum structured data needed to represent `Prescription Upload` safely:
...
Show a calm empty state explaining that no `Prescription Upload` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
