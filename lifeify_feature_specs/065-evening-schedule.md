...
| Version | V1 |
| Category | Medication |
| Feature Key | `evening_schedule` |

## Purpose

Evening Schedule supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Evening Schedule` safely:
...
Show a calm empty state explaining that no `Evening Schedule` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
