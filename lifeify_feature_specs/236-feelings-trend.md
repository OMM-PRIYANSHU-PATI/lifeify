...
| Feature Key | `feelings_trend` |

## Purpose

Feelings Trend supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Feelings Trend` safely:
...
Show a calm empty state explaining that no `Feelings Trend` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
