# Feature 111: Cardiovascular Tracking
...
| Feature Key | `cardiovascular_tracking` |

## Purpose

Cardiovascular Tracking captures longitudinal health information so the user can build a useful timeline without depending on a single external platform.
...
The feature requires the minimum structured data needed to represent `Cardiovascular Tracking` safely:
...
Billing provider records and LIFEIFY entitlement records.

## Fallback Data Source

Read-only free-plan state if billing confirmation is unavailable.
...
No medical quiz is needed; onboarding may explain plan preferences.
...
Show a calm empty state explaining that no `Cardiovascular Tracking` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
