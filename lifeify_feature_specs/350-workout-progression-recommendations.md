# Feature 350: Workout Progression Recommendations
...
| Version | V3 |
| Category | AI Fitness |
| Feature Key | `workout_progression_recommendations` |

## Purpose

Workout Progression Recommendations helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Workout Progression Recommendations` safely:
...
Show a calm empty state explaining that no `Workout Progression Recommendations` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
