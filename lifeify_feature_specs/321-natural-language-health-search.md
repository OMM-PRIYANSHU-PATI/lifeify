# Feature 321: Natural-Language Health Search
...
| Feature Key | `natural_language_health_search` |

## Purpose

Natural-Language Health Search helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Natural-Language Health Search` safely:
...
Show a calm empty state explaining that no `Natural-Language Health Search` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
