...
Water gives the user an at-a-glance view of current health status, progress, and missing actions.
...
The feature requires the minimum structured data needed to represent `Water` safely:
...
Additional fields and entities should be scoped to the category `Dashboard` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Water` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
