# Feature 269: Doctor Shared Reports
...
| Feature Key | `doctor_shared_reports` |

## Purpose

Doctor Shared Reports lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Doctor Shared Reports` safely:
...
Show a calm empty state explaining that no `Doctor Shared Reports` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
