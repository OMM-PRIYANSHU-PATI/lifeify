# Feature 250: Additional Family Members
...
| Feature Key | `additional_family_members` |

## Purpose

Additional Family Members lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Additional Family Members` safely:
...
Additional fields and entities should be scoped to the category `Family & Caregivers` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Additional Family Members` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
