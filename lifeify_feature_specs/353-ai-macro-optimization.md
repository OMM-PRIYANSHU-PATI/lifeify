# Feature 353: AI Macro Optimization
...
| Feature Key | `ai_macro_optimization` |

## Purpose

AI Macro Optimization helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `AI Macro Optimization` safely:
...
Show a calm empty state explaining that no `AI Macro Optimization` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
