# Feature 323: Personalized Health Explanations
...
| Feature Number | 323 |
| Version | V3 |
| Category | AI Assistant & Memory |
| Feature Key | `personalized_health_explanations` |

## Purpose

Personalized Health Explanations helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Personalized Health Explanations` safely:
...
Additional fields and entities should be scoped to the category `AI Assistant & Memory` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Personalized Health Explanations` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
