# Feature 411: AI Evaluation Framework
...
| Feature Key | `ai_evaluation_framework` |

## Purpose

AI Evaluation Framework helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `AI Evaluation Framework` safely:
...
Additional fields and entities should be scoped to the category `AI Governance` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `AI Evaluation Framework` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
