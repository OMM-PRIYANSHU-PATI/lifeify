# Feature 404: Clinical Knowledge Retrieval
...
| Feature Key | `clinical_knowledge_retrieval` |

## Purpose

Clinical Knowledge Retrieval helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Clinical Knowledge Retrieval` safely:
...
Show a calm empty state explaining that no `Clinical Knowledge Retrieval` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
