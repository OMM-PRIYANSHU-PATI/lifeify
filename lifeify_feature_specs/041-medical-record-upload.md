# Feature 041: Medical Record Upload
...
| Feature Key | `medical_record_upload` |

## Purpose

Medical Record Upload helps organize medical documents and convert user-confirmed extracted information into structured records.
...
The feature requires the minimum structured data needed to represent `Medical Record Upload` safely:
...
Show a calm empty state explaining that no `Medical Record Upload` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
