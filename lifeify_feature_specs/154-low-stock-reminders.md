# Feature 154: Low-Stock Reminders
...
| Feature Key | `low_stock_reminders` |

## Purpose

Low-Stock Reminders supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Low-Stock Reminders` safely:
...
Show a calm empty state explaining that no `Low-Stock Reminders` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
