# Feature 383: Medication Adherence Prediction
...
| Feature Number | 383 |
| Version | V3 |
| Category | ML Modeling & Prediction |
| Feature Key | `medication_adherence_prediction` |

## Purpose

Medication Adherence Prediction helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Medication Adherence Prediction` safely:
...
Additional fields and entities should be scoped to the category `ML Modeling & Prediction` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Medication Adherence Prediction` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
