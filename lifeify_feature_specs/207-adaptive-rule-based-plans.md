# Feature 207: Adaptive Rule-Based Plans
...
| Feature Key | `adaptive_rule_based_plans` |

## Purpose

Adaptive Rule-Based Plans contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Adaptive Rule-Based Plans` safely:
...
Show a calm empty state explaining that no `Adaptive Rule-Based Plans` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
