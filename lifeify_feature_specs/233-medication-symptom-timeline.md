# Feature 233: Medication/Symptom Timeline
...
| Feature Key | `medication_symptom_timeline` |

## Purpose

Medication/Symptom Timeline supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Medication/Symptom Timeline` safely:
...
Show a calm empty state explaining that no `Medication/Symptom Timeline` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
