# Feature 252: Family Health Summary
...
| Feature Key | `family_health_summary` |

## Purpose

Family Health Summary lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Family Health Summary` safely:
...
Show a calm empty state explaining that no `Family Health Summary` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
