# Feature 396: Personalized Daily Actions
...
| Feature Key | `personalized_daily_actions` |

## Purpose

Personalized Daily Actions helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Personalized Daily Actions` safely:
...
Show a calm empty state explaining that no `Personalized Daily Actions` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
