# Feature 277: Screen Reader Support
...
| Feature Key | `screen_reader_support` |

## Purpose

Screen Reader Support contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Screen Reader Support` safely:
...
Additional fields and entities should be scoped to the category `Accessibility & Localization` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Screen Reader Support` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
