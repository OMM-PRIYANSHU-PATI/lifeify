# Feature 360: AI Medication Assistant
...
| Feature Key | `ai_medication_assistant` |

## Purpose

AI Medication Assistant helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `AI Medication Assistant` safely:
...
Show a calm empty state explaining that no `AI Medication Assistant` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
