# Feature 095: Previous-Response-Based Questions
...
| Feature Key | `previous_response_based_questions` |

## Purpose

Previous-Response-Based Questions contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Previous-Response-Based Questions` safely:
...
Show a calm empty state explaining that no `Previous-Response-Based Questions` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
