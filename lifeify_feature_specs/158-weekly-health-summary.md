# Feature 158: Weekly Health Summary
...
| Feature Key | `weekly_health_summary` |

## Purpose

Weekly Health Summary contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Weekly Health Summary` safely:
...
Show a calm empty state explaining that no `Weekly Health Summary` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
