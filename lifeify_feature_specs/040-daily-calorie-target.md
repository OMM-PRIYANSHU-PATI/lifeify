# Feature 040: Daily Calorie Target
...
| Feature Key | `daily_calorie_target` |

## Purpose

Daily Calorie Target captures longitudinal health information so the user can build a useful timeline without depending on a single external platform.
...
The feature requires the minimum structured data needed to represent `Daily Calorie Target` safely:
...
Show a calm empty state explaining that no `Daily Calorie Target` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
