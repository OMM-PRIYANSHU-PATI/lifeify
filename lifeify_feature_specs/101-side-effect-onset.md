...
| Feature Key | `side_effect_onset` |

## Purpose

Side-Effect Onset contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Side-Effect Onset` safely:
...
Show a calm empty state explaining that no `Side-Effect Onset` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
