...
Steps gives the user an at-a-glance view of current health status, progress, and missing actions.
...
The feature requires the minimum structured data needed to represent `Steps` safely:
...
Show a calm empty state explaining that no `Steps` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
