# Feature 218: Food Substitutions
...
| Feature Key | `food_substitutions` |

## Purpose

Food Substitutions contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Food Substitutions` safely:
...
Show a calm empty state explaining that no `Food Substitutions` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
