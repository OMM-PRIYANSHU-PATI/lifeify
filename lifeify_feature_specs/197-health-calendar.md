...
| Feature Key | `health_calendar` |

## Purpose

Health Calendar contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Health Calendar` safely:
...
Show a calm empty state explaining that no `Health Calendar` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
