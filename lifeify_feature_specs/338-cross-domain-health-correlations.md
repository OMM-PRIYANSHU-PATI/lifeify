# Feature 338: Cross-Domain Health Correlations
...
| Feature Key | `cross_domain_health_correlations` |

## Purpose

Cross-Domain Health Correlations helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Cross-Domain Health Correlations` safely:
...
Show a calm empty state explaining that no `Cross-Domain Health Correlations` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
