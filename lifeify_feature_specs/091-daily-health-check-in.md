# Feature 091: Daily Health Check-in
...
| Feature Key | `daily_health_check_in` |

## Purpose

Daily Health Check-in contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Daily Health Check-in` safely:
...
Show a calm empty state explaining that no `Daily Health Check-in` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
