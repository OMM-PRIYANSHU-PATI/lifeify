# Feature 151: Secure Report Sharing
...
| Feature Key | `secure_report_sharing` |

## Purpose

Secure Report Sharing lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Secure Report Sharing` safely:
...
Show a calm empty state explaining that no `Secure Report Sharing` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
