# Feature 367: Drug Interaction Engine
...
| Feature Key | `drug_interaction_engine` |

## Purpose

Drug Interaction Engine helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Drug Interaction Engine` safely:
...
Show a calm empty state explaining that no `Drug Interaction Engine` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
