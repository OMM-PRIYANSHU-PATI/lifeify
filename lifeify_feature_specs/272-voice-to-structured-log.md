# Feature 272: Voice-to-Structured-Log
...
| Feature Key | `voice_to_structured_log` |

## Purpose

Voice-to-Structured-Log contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Voice-to-Structured-Log` safely:
...
Show a calm empty state explaining that no `Voice-to-Structured-Log` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
