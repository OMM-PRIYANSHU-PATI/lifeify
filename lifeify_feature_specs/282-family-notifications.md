# Feature 282: Family Notifications
...
| Feature Key | `family_notifications` |

## Purpose

Family Notifications lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Family Notifications` safely:
...
Show a calm empty state explaining that no `Family Notifications` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
