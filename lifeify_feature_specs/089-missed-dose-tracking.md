# Feature 089: Missed-Dose Tracking
...
| Feature Key | `missed_dose_tracking` |

## Purpose

Missed-Dose Tracking captures longitudinal health information so the user can build a useful timeline without depending on a single external platform.
...
The feature requires the minimum structured data needed to represent `Missed-Dose Tracking` safely:
...
Additional fields and entities should be scoped to the category `Medication Adherence` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Missed-Dose Tracking` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
