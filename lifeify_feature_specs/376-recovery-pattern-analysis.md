# Feature 376: Recovery Pattern Analysis
...
| Feature Key | `recovery_pattern_analysis` |

## Purpose

Recovery Pattern Analysis helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Recovery Pattern Analysis` safely:
...
Show a calm empty state explaining that no `Recovery Pattern Analysis` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
