# Feature 276: Keyboard Accessibility
...
| Feature Key | `keyboard_accessibility` |

## Purpose

Keyboard Accessibility lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Keyboard Accessibility` safely:
...
Show a calm empty state explaining that no `Keyboard Accessibility` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
