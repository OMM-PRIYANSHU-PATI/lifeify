# Feature 344: AI Weekly Health Review
...
| Feature Key | `ai_weekly_health_review` |

## Purpose

AI Weekly Health Review helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `AI Weekly Health Review` safely:
...
Show a calm empty state explaining that no `AI Weekly Health Review` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
