# Feature 137: Secure Emergency Link
...
| Version | V1 |
| Category | Emergency |
| Feature Key | `secure_emergency_link` |

## Purpose

Secure Emergency Link lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Secure Emergency Link` safely:
...
Show a calm empty state explaining that no `Secure Emergency Link` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
