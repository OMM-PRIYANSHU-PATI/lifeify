# Feature 265: Doctor Access Revocation
...
| Feature Key | `doctor_access_revocation` |

## Purpose

Doctor Access Revocation lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Doctor Access Revocation` safely:
...
Show a calm empty state explaining that no `Doctor Access Revocation` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
