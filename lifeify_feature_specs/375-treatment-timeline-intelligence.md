# Feature 375: Treatment Timeline Intelligence
...
| Feature Number | 375 |
| Version | V3 |
| Category | AI Recovery Intelligence |
| Feature Key | `treatment_timeline_intelligence` |

## Purpose

Treatment Timeline Intelligence helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Treatment Timeline Intelligence` safely:
...
Additional fields and entities should be scoped to the category `AI Recovery Intelligence` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Treatment Timeline Intelligence` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
