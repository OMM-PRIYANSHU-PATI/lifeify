# Feature 247: Medication-Associated Symptoms
...
| Version | V2 |
| Category | Symptoms |
| Feature Key | `medication_associated_symptoms` |

## Purpose

Medication-Associated Symptoms supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Medication-Associated Symptoms` safely:
...
Show a calm empty state explaining that no `Medication-Associated Symptoms` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
