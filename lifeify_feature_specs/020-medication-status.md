...
| Version | V1 |
| Category | Dashboard |
| Feature Key | `medication_status` |

## Purpose

Medication Status gives the user an at-a-glance view of current health status, progress, and missing actions.
...
The feature requires the minimum structured data needed to represent `Medication Status` safely:
...
Show a calm empty state explaining that no `Medication Status` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
