# Feature 379: Personalized Anomaly Alerts
...
| Feature Key | `personalized_anomaly_alerts` |

## Purpose

Personalized Anomaly Alerts helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Personalized Anomaly Alerts` safely:
...
Show a calm empty state explaining that no `Personalized Anomaly Alerts` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
