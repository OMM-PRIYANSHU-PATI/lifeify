# Feature 084: Drug Reference Database
...
| Feature Key | `drug_reference_database` |

## Purpose

Drug Reference Database supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Drug Reference Database` safely:
...
Show a calm empty state explaining that no `Drug Reference Database` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
