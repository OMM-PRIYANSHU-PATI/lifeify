# Feature 348: Recovery-Aware Recommendations
...
| Version | V3 |
| Category | AI Fitness |
| Feature Key | `recovery_aware_recommendations` |

## Purpose

Recovery-Aware Recommendations helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Recovery-Aware Recommendations` safely:
...
Show a calm empty state explaining that no `Recovery-Aware Recommendations` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
