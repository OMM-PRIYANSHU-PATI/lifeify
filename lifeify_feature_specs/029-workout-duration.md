...
| Feature Key | `workout_duration` |

## Purpose

Workout Duration captures longitudinal health information so the user can build a useful timeline without depending on a single external platform.
...
The feature requires the minimum structured data needed to represent `Workout Duration` safely:
...
Show a calm empty state explaining that no `Workout Duration` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
