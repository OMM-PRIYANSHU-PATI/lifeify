# Feature 328: Longitudinal Report Analysis
...
| Feature Key | `longitudinal_report_analysis` |

## Purpose

Longitudinal Report Analysis helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Longitudinal Report Analysis` safely:
...
Show a calm empty state explaining that no `Longitudinal Report Analysis` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
