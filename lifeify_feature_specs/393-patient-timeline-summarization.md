# Feature 393: Patient Timeline Summarization
...
| Feature Key | `patient_timeline_summarization` |

## Purpose

Patient Timeline Summarization helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Patient Timeline Summarization` safely:
...
Show a calm empty state explaining that no `Patient Timeline Summarization` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
