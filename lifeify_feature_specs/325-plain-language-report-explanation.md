# Feature 325: Plain-Language Report Explanation
...
| Feature Key | `plain_language_report_explanation` |

## Purpose

Plain-Language Report Explanation helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Plain-Language Report Explanation` safely:
...
Additional fields and entities should be scoped to the category `AI Report Analysis` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Plain-Language Report Explanation` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
