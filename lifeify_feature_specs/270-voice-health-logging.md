# Feature 270: Voice Health Logging
...
| Feature Number | 270 |
| Version | V2 |
| Category | Voice Logging |
| Feature Key | `voice_health_logging` |

## Purpose

Voice Health Logging captures longitudinal health information so the user can build a useful timeline without depending on a single external platform.
...
The feature requires the minimum structured data needed to represent `Voice Health Logging` safely:
...
Additional fields and entities should be scoped to the category `Voice Logging` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Voice Health Logging` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
