# Feature 107: Doctor-Shareable ADR Summary
...
| Feature Number | 107 |
| Version | V1 |
| Category | Side Effects & ADR |
| Feature Key | `doctor_shareable_adr_summary` |

## Purpose

Doctor-Shareable ADR Summary lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Doctor-Shareable ADR Summary` safely:
...
Additional fields and entities should be scoped to the category `Side Effects & ADR` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Doctor-Shareable ADR Summary` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
