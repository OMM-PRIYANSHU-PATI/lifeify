# Feature 377: Personal Baseline Modeling
...
| Feature Key | `personal_baseline_modeling` |

## Purpose

Personal Baseline Modeling helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Personal Baseline Modeling` safely:
...
Show a calm empty state explaining that no `Personal Baseline Modeling` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
