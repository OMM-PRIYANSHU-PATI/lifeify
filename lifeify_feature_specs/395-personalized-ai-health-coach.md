# Feature 395: Personalized AI Health Coach
...
| Feature Number | 395 |
| Version | V3 |
| Category | AI Coaching & Context |
| Feature Key | `personalized_ai_health_coach` |

## Purpose

Personalized AI Health Coach helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Personalized AI Health Coach` safely:
...
Additional fields and entities should be scoped to the category `AI Coaching & Context` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Personalized AI Health Coach` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
