# Feature 133: Emergency Medications
...
| Version | V1 |
| Category | Emergency |
| Feature Key | `emergency_medications` |

## Purpose

Emergency Medications supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Emergency Medications` safely:
...
Additional fields and entities should be scoped to the category `Emergency` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Emergency Medications` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
