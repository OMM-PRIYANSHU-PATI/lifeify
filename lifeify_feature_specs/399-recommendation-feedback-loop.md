# Feature 399: Recommendation Feedback Loop
...
| Feature Key | `recommendation_feedback_loop` |

## Purpose

Recommendation Feedback Loop helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Recommendation Feedback Loop` safely:
...
Show a calm empty state explaining that no `Recommendation Feedback Loop` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
