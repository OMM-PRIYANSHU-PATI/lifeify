# Feature 324: AI Report Analysis
...
| Feature Key | `ai_report_analysis` |

## Purpose

AI Report Analysis helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `AI Report Analysis` safely:
...
Show a calm empty state explaining that no `AI Report Analysis` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
