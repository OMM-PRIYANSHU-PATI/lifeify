...
| Version | V1 |
| Category | Recovery |
| Feature Key | `symptom_progress` |

## Purpose

Symptom Progress contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Symptom Progress` safely:
...
Additional fields and entities should be scoped to the category `Recovery` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Symptom Progress` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
