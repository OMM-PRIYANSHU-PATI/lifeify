# Feature 317: AI Health Assistant
...
| Feature Key | `ai_health_assistant` |

## Purpose

AI Health Assistant helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `AI Health Assistant` safely:
...
Show a calm empty state explaining that no `AI Health Assistant` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
