# Feature 251: Family Health Dashboard
...
| Feature Key | `family_health_dashboard` |

## Purpose

Family Health Dashboard lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Family Health Dashboard` safely:
...
Show a calm empty state explaining that no `Family Health Dashboard` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
