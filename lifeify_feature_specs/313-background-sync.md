...
| Feature Key | `background_sync` |

## Purpose

Background Sync contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Background Sync` safely:
...
Show a calm empty state explaining that no `Background Sync` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
