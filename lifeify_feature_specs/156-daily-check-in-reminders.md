# Feature 156: Daily Check-in Reminders
...
| Feature Key | `daily_check_in_reminders` |

## Purpose

Daily Check-in Reminders contributes to the LIFEIFY Personal Health OS by collecting, organizing, or acting on user-confirmed health data.
...
The feature requires the minimum structured data needed to represent `Daily Check-in Reminders` safely:
...
Show a calm empty state explaining that no `Daily Check-in Reminders` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
