# Feature 234: Weekly Medication Check-in
...
| Feature Key | `weekly_medication_check_in` |

## Purpose

Weekly Medication Check-in supports safe medication organization, schedule tracking, adherence visibility, and user-confirmed treatment records.
...
The feature requires the minimum structured data needed to represent `Weekly Medication Check-in` safely:
...
Show a calm empty state explaining that no `Weekly Medication Check-in` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
