# Feature 400: Personalized Health Profile
...
| Feature Key | `personalized_health_profile` |

## Purpose

Personalized Health Profile helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Personalized Health Profile` safely:
...
Show a calm empty state explaining that no `Personalized Health Profile` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
