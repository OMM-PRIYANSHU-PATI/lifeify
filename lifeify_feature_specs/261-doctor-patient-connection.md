# Feature 261: Doctor-Patient Connection
...
| Feature Key | `doctor_patient_connection` |

## Purpose

Doctor-Patient Connection lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Doctor-Patient Connection` safely:
...
Show a calm empty state explaining that no `Doctor-Patient Connection` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
