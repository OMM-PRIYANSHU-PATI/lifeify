# Feature 267: Doctor Appointments
...
| Feature Key | `doctor_appointments` |

## Purpose

Doctor Appointments lets the user control how sensitive health information is prepared, shared, viewed, and revoked.
...
The feature requires the minimum structured data needed to represent `Doctor Appointments` safely:
...
Show a calm empty state explaining that no `Doctor Appointments` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
