# Feature 394: Natural-Language Clinical Data Search
...
| Feature Number | 394 |
| Version | V3 |
| Category | AI Doctor Support |
| Feature Key | `natural_language_clinical_data_search` |

## Purpose

Natural-Language Clinical Data Search helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `Natural-Language Clinical Data Search` safely:
...
Additional fields and entities should be scoped to the category `AI Doctor Support` and linked by stable IDs with source metadata.
...
Show a calm empty state explaining that no `Natural-Language Clinical Data Search` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
