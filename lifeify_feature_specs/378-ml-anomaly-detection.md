# Feature 378: ML Anomaly Detection
...
| Feature Key | `ml_anomaly_detection` |

## Purpose

ML Anomaly Detection helps the user understand their health data through an AI layer that operates only on validated, permissioned, and provenance-aware records.
...
The feature requires the minimum structured data needed to represent `ML Anomaly Detection` safely:
...
Show a calm empty state explaining that no `ML Anomaly Detection` data exists yet, then offer the safest next action: connect a supported source, enter data manually, use quick log, answer a short quiz, upload a document, or invite an approved contributor where applicable.
